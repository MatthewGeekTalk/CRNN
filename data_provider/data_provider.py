...
class TextDataProvider(object):
    ...
    def __init__(self, dataset_dir, annotation_name, validation_set=None, validation_split=None, shuffle=None,
        ...
        with open(test_anno_path, 'r',encoding='utf-8-sig') as anno_file:
            info = np.array([tmp.strip().split() for tmp in anno_file.readlines()])
            test_images = np.array([cv2.imread(ops.join(self.__test_dataset_dir, tmp[0]), cv2.IMREAD_COLOR)
                                   for tmp in info[:]])
            # images = []
            # i = 0
            # for tmp1 in info[:]:
            ...
            #     if i == 1290:
            #         print(path)
            #     img = cv2.imread(ops.join(self.__test_dataset_dir, path ), cv2.IMREAD_COLOR)
            #     images.append(img)
            #     i += 1
            # test_images = np.array(images)
            test_labels = np.array([tmp[1] for tmp in info[:]])

            test_imagenames = np.array([ops.basename(tmp[0]) for tmp in info[:]])
        ...
        with open(train_anno_path, 'r',encoding='utf-8-sig') as anno_file:
            info = np.array([tmp.strip().split() for tmp in anno_file.readlines()])
            train_images = np.array([cv2.imread(ops.join(self.__train_dataset_dir, tmp[0]), cv2.IMREAD_COLOR)
                                    for tmp in info[:]])
            train_labels = np.array([tmp[1] for tmp in info[:]])
            train_imagenames = np.array([ops.basename(tmp[0]) for tmp in info[:]])
