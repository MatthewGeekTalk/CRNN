...
def train_shadownet(dataset_dir, weights_path=None):
    ...
    inputdata, input_labels, input_imagenames = tf.train.shuffle_batch(
        tensors=[images, labels, imagenames], batch_size=32, capacity=50000, min_after_dequeue=10000, num_threads=4)

    inputdata = tf.cast(x=inputdata, dtype=tf.float32)

    # initializa the net model
    shadownet = crnn_model.ShadowNet(phase='Train', hidden_nums=256, layers_nums=2, seq_length=25, num_classes=62)
    ...
    model_save_path = ops.join(model_save_dir, 'shadownet.ckpt')
